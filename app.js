...
let relays = [], inbounds = [], users = [], agents = [], filter = 'all', systemInfo = null, traffic = null;
...
function renderInbounds() {
  $('#inboundCards').innerHTML = inbounds.length ? inbounds.map(inbound => { const state = inbound.status || 'stopped'; const stateText = state === 'running' ? zh.run : state === 'starting' ? '下发中' : state === 'error' ? '异常' : state === 'offline' ? 'Agent 离线' : zh.stop; const executor = inbound.agentId ? `Agent · ${inbound.agentName || inbound.agentId}` : '本机 Xray'; return `<article class="node-card"><div class="inbound-title"><b>◈</b><div><strong>${esc(inbound.name)}</strong><small>${esc(inbound.template || inbound.protocol || inbound.protocolCode)} · ${esc(executor)}</small></div></div><div class="inbound-meta"><b>${zh.listen}</b>${esc(inbound.port)}</div><div class="inbound-meta"><b>${zh.server}</b>${esc(inbound.serverAddress || '-')}</div><div class="status ${esc(state)}">${stateText}<br><small title="${esc(inbound.lastError || '')}">${esc(inbound.lastError || `${zh.security}: ${inbound.security}`)}</small></div><div class="node-link wide"><b>${zh.link}</b><code>${esc(inbound.shareLink || '')}</code></div><div class="inbound-meta wide"><b>${zh.remark}</b>${esc(inbound.remark || zh.noRemark)}</div><details class="node-json wide"><summary>Xray JSON</summary><pre>${esc(JSON.stringify(inbound.xray || {}, null, 2))}</pre></details><div class="inbound-actions wide"><button class="btn" onclick="showInboundQr(${inbound.id})">二维码</button><button class="btn" onclick="copyInbound(${inbound.id}, 'shareLink')">${zh.copyLink}</button><button class="btn" onclick="copyInbound(${inbound.id}, 'xray')">${zh.copyJson}</button><button class="btn" onclick="toggleInbound(${inbound.id})">${state === 'running' || state === 'starting' ? zh.pause : zh.enable}</button><button class="btn danger" onclick="deleteInbound(${inbound.id})">${zh.del}</button></div></article>`; }).join('') : `<article>${zh.emptyInbound}</article>`;
...
async function copyText(value) { if (navigator.clipboard) { await navigator.clipboard.writeText(value); alert(zh.copied); } else prompt(zh.copyLink, value); }
...
$('#newAgent').onclick = () => { $('#agentControllerUrl').value = location.origin; $('#agentModal').showModal(); };$('#newInbound').onclick = () => { populateInboundAgents(); applyInboundTemplate(true); const address = $('#inboundForm').elements.serverAddress; if (address && !address.value && systemInfo?.network?.publicAddress) address.value = systemInfo.network.publicAddress; $('#inboundModal').showModal(); };
...
$('#copyAgentCommand').onclick = async () => { try { await copyText($('#agentCommand').textContent); } catch { prompt('启动命令', $('#agentCommand').textContent); } };$('#inboundForm').onsubmit = async event => { event.preventDefault(); try { await api('/api/inbounds', { method: 'POST', body: JSON.stringify(Object.fromEntries(new FormData(event.target))) }); event.target.reset(); $('#inboundModal').close(); await load(); } catch (error) { alert(error.message); } };
