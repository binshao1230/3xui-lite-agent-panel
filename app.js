...
$('#copyAgentCommand').onclick = async () => { try { await copyText($('#agentCommand').textContent); } catch { prompt('启动命令', $('#agentCommand').textContent); } };$('#import3xuiForm').onsubmit = async event => { event.preventDefault(); try { await api('/api/inbounds/import-3xui', { method: 'POST', body: JSON.stringify(Object.fromEntries(new FormData(event.target))) }); event.target.reset(); $('#import3xuiModal').close(); await load(); } catch (error) { alert(error.message); } };$('#inboundForm').onsubmit = async event => { event.preventDefault(); try { const target = editingInboundId ? '/api/inbounds/' + editingInboundId : '/api/inbounds'; await api(target, { method: editingInboundId ? 'PATCH' : 'POST', body: JSON.stringify(Object.fromEntries(new FormData(event.target))) }); editingInboundId = null; event.target.reset(); $('#inboundProtocol').disabled = false; $('#inboundModal').close(); await load(); } catch (error) { alert(error.message); } };
