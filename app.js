...
function renderUsers() {
  $('#userCards').innerHTML = users.length ? users.map(user => {
    ...
    return `<article><div class="user-main"><b>◎</b><div><strong>${esc(user.name)}</strong><small>${esc(user.email)}</small>${user.access?.[0] ? `<small class="access-label">已绑定：${esc(user.access[0].protocol)}</small>` : ''}</div></div><div class="quota"><span><i style="width:${percent}%"></i></span><small>${used.toFixed(1)} / ${limit} GB</small></div><div class="status ${esc(user.status)}">${expired ? '已到期' : user.status === 'running' ? zh.run : zh.stop}<br><small>${esc(user.expire || '长期有效')}</small></div><div class="inbound-actions">${user.access?.[0]?.link ? `<button class="btn" onclick="copyUserLink(${user.id})">节点链接</button>` : ''}<button class="btn" onclick="editUser(${user.id})">编辑</button><button class="btn" onclick="toggleUser(${user.id})">${user.status === 'running' ? zh.pause : expired ? '更新到期日' : zh.enable}</button><button class="btn danger" onclick="deleteUser(${user.id})">${zh.del}</button></div></article>`;
...
function closeSidebar() {
  $('.sidebar').classList.remove('open'); document.body.classList.remove('nav-open'); $('#menu').setAttribute('aria-expanded', 'false'); $('#menu').setAttribute('aria-label', '打开菜单');
}
function activatePage(id) {
  ...
  $('#crumb').innerHTML = `控制台 / <strong>${pageNames[id]}</strong>`; history.replaceState(null, '', `#${id}`); closeSidebar();
}
...
$('#newRelay').onclick = () => { editingRelayId = null; const form = $('#form'); form.reset(); form.elements.bindAddress.value = '0.0.0.0'; populateRelayAgents(); $('#relayModalEyebrow').textContent = 'NEW RELAY'; $('#relayModalTitle').textContent = '创建端口转发'; $('#relaySubmit').textContent = '创建并启用'; $('#modal').showModal(); };
...
$('#menu').onclick = () => { const open = !$('.sidebar').classList.contains('open'); $('.sidebar').classList.toggle('open', open); document.body.classList.toggle('nav-open', open); $('#menu').setAttribute('aria-expanded', String(open)); $('#menu').setAttribute('aria-label', open ? '关闭菜单' : '打开菜单'); };
